 ...
 */
public class JimpleSparseCFGBuilder implements SparseCFGBuilder<Unit, SootMethod, DFF> {

    private final static Logger LOGGER = Logger.getLogger(JimpleSparseCFGBuilder.class.getName());

    private boolean addedInvoke = false;

    private boolean enableExceptions;

    public JimpleSparseCFGBuilder(boolean enableExceptions) {
    ...
    private boolean shouldKeepStmt(Unit unit, DFF d, SootMethod m, DirectedGraph<Unit> graph) {
        ...
        if (unit.toString().equals(d.getGeneratedAt().toString())) {
